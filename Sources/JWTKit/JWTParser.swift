...
public struct JWTParser: Sendable {
    ...
    public init(token: some DataProtocol) throws {
    ...
    public func header(jsonDecoder: any JWTJSONDecoder = .defaultForJWT) throws -> JWTHeader {
        try jsonDecoder
            .decode(JWTHeader.self, from: .init(encodedHeader.base64URLDecodedBytes()))
    }

    public func payload<Payload>(as _: Payload.Type, jsonDecoder: any JWTJSONDecoder = .defaultForJWT) throws -> Payload
    ...
    public func verify(using algorithm: JWTAlgorithm) throws {
