...
class JWTKitTests: XCTestCase {
    ...
    func testAlgorithmInJWTHeaderOnly() async throws {
        ...
        // sign jwt
        let keyCollection = try await JWTKeyCollection().addRS256(key: RSAKey(
            modulus: modulus,
            exponent: exponent,
    ...
    func testAppStoreJWT() async throws {
        ...
        struct StoreKitPayload: ValidationTimePayload {
            func verify(using _: JWTKit.JWTAlgorithm) async throws {}
        ...
        await XCTAssertNoThrowAsync(try await verifier.verifyJWS(data, as: StoreKitPayload.self))
    }
}
