...
@Suite("Performance", .serialized, .disabled())
...
private func report(_ name: String, _ perCall: Duration, _ instructions: UInt64) {
    ...
    let instructionColumn =
        instructions == 0 ? "" : "   \(String(repeating: " ", count: max(0, 10 - String(instructions).count)))\(instructions) instr/op"
