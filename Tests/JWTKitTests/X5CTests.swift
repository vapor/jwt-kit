...
final class X5CTests: XCTestCase {
    ...
    func testAppStoreJWT() async throws {
        ...
        do {
            payload = try await verifier.verifyJWS(data, as: StoreKitPayload.self, jsonDecoder: jsonDecoder)
        } catch {
            XCTFail("Failed with error: \(error.localizedDescription)")
        }

        if let payload = payload {
            XCTAssertEqual(payload.data.appAppleId, 1234)
            XCTAssertEqual(payload.data.environment, "Sandbox")
        } else {
            XCTFail("Payload was not initialized")
        }
    }
}
